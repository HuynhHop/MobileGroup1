...
class UserController {
  ...
  async resetPassword(req, res, next) {
    ...
    async function getUserFromToken(req, res) {
      try {
        ...
        const userId = req.user.id; // Giả định bạn lưu ID của người dùng trong token

        // Lấy thông tin người dùng từ database
        const user = await User.findById(userId).select(
          "-password -refreshToken"
        ); // Không trả về password và refreshToken
        if (!user) {
          return res
            .status(404)
  ...
  async function getUserFromToken(req, res) {
    try {
        ...
        // Lấy thông tin người dùng từ database
        const user = await User.findById(userId).select('-password -refreshToken'); // Không trả về password và refreshToken
        if (!user) {
